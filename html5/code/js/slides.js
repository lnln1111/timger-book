...
var PERMANENT_URL_PREFIX = 'http://html5ppt.sinaapp.com/';
...
function disableSlideFrames(no) {
  ...
  // check
  var hdl = el.getAttribute('onslideleave');
...
function enableSlideFrames(no) {
  ...
  // check
  var hdl = el.getAttribute('onslideenter'); 
  if (no != curSlide && hdl && (hdl!="") ){
	  return;
  } 
  var frames = el.getElementsByTagName('iframe');
  for (var i = 0, frame; frame = frames[i]; i++) {
...
function iframeEnter(){
	console.info('enter: '+curSlide);
	enableSlideFrames(curSlide);
}
function iframeLeave(){
	// 当前的id 已经换过了，所以需要回退一下
